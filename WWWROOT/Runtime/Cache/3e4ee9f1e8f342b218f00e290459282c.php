<?php if (!defined('THINK_PATH')) exit();?><!DOCTYPE html PUBLIC "-//W3C//DTD XHTML 1.0 Transitional//EN" "http://www.w3.org/TR/xhtml1/DTD/xhtml1-transitional.dtd"><html><head><meta http-equiv="Content-Type" content="text/html; charset=UTF-8"><script type="text/javascript" src="./Tpl/static/js/jquery.js"></script><script type="text/javascript" src="./Tpl/static/js/asyncbox/AsyncBox.js"></script><link rel="stylesheet" type="text/css" href="./Tpl/static/js/asyncbox/skins/ZCMS/asyncbox.css" /><link rel="stylesheet" type="text/css" href="./Tpl/static/css/css.css" /><style type="text/css"><!--
        ...
        $('#add').click(function() {
            show("<?php echo U('User/add');?>", '增加用户');
        });
    });
    function edit(id) {
        var url = '<?php echo U('User/edit?id');?>' + id;
                show(url, '修改用户');
    }
    function del(id) {
        asyncbox.confirm('您确定要删除当前信息吗？删除后不可恢复！', '删除确认', function(action) {
            ...
            if (action == 'ok') {
                $.post('<?php echo U('User/del');?>', 'id=' + id, function(e) {
                    if (e == '1') { 
...
</script><table width="90%" id="table"  border="1" class="t1"><thead><th width="10%">所属类别</th><th width="20%">部门名称</th><th width="20%">用户名</th><th width="20%">密码</th><th width="8%">权限</th><th width="20%">操作</th></thead><?php if($data){ if(is_array($data)): foreach($data as $key=>$value): ?><tr class="a1" flag='U_<?php echo ($value['id']); ?>'><td><?php echo ($value['sort_name']); ?></td><td><?php echo ($value['class_name']); ?></td><td><?php echo ($value['name']); ?></td><td><?php echo ($value['pass']); ?></td><td><?php echo ($value['unit']); ?></td><td><input type="button" onclick="edit(<?php echo ($value['id']); ?>)" value="修改"/><input type="button" onclick="del(<?php echo ($value['id']); ?>)" value="删除"/></td></tr><?php endforeach; endif; }else{ ?><tr><td colspan="6">没有任何信息，请添加</td></tr><?php }?><tr><td colspan="6"><input type="button" value='增加' id ='add'></td></tr></table></div></body></html>